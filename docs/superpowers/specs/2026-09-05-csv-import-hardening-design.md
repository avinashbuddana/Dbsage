...
- **Phase B — Mandatory `created_at`/`updated_at` on created tables** — not started.
- **Phase C — Schema-aware datatype transformation, validation, preview, strict/flexible modes** (own detailed spec once B ships; outlined here at architecture level so A/B's schema choices don't conflict with it later) — not started.
